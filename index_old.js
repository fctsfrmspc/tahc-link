...
bot.on("ready", () => {
	...
	//times(25,()=>console.log(rando(6,1)))
	get_instagram()
  getSoSo()
})
...
app.get("/", (request, response) => {
	rightnow = new Date();
	console.log(rightnow.toLocaleString() + " Ping Received ["+request.headers["x-forwarded-for"]+"]");
	response.sendStatus(200);
});
//app.listen(process.env.PORT);
//var timeout = setInterval(() => {
	//request.get(`http://${process.env.PROJECT_DOMAIN}.glitch.me/`);
//}, 280000);
